...
import { ExternalLink, MessageSquare, Calendar, CircleDot } from "lucide-react";
...
function IssuesContent() {
  ...
  const state = (searchParams.get("state") as "opened" | "closed" | "all") || "opened";
  ...
  const fetchIssues = useCallback(async (p: number, s: string) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/gitlab/issues?state=${s}&page=${p}`);
  ...
  useEffect(() => { fetchIssues(page, state); }, [page, state, fetchIssues]);

  const setState = (s: string) => router.push(`/issues?state=${s}`);

  return (
    <div>
      <Header
        title="Issues"
        subtitle={total > 0 ? `${total} issue(s)` : "Issues du groupe"}
      />
      ...
      <div className="bg-white rounded-2xl overflow-hidden"
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <CircleDot className="w-10 h-10 mx-auto mb-3 opacity-20" style={{ color: "#8E89B8" }} />
            <p className="font-semibold text-sm" style={{ color: "#4A4580" }}>Aucune issue</p>
          </div>
        ) : (
          <div>
            {issues.map((issue, idx) => (
              <div
                ...
                style={{ borderBottom: idx < issues.length - 1 ? "1px solid #EDEAF8" : "none" }}
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mb-2">
                      <Badge variant={issue.state === "opened" ? "success" : "neutral"}>
                        {issue.state === "opened" ? "Ouverte" : "Fermée"}
                      </Badge>
                      {issue.milestone && (
                        <Badge variant="purple">{issue.milestone.title}</Badge>
                      )}
                    ...
                    >
                      <span className="font-mono text-xs mr-1.5" style={{ color: "#8E89B8" }}>#{issue.iid}</span>
                      {issue.title}
                    </a>

                    {/* Meta */}
                    <div className="flex items-center gap-2 mt-1.5 text-xs flex-wrap" style={{ color: "#8E89B8" }}>
                      <span>par <span className="font-semibold" style={{ color: "#4A4580" }}>{issue.author.name}</span></span>
